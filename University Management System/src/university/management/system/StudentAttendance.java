...
public class StudentAttendance extends JFrame implements ActionListener{
    ...
    StudentAttendance(){
        ...
        setLayout(new GridLayout(4,2,50,50));//4 rows 2 columns
        c2 = new Choice();
        try{
            conn c = new conn();
            ResultSet rs = c.s.executeQuery("select * from student");
            while(rs.next()){
                c2.add(rs.getString("rollno"));    //adding all roll no.s in choices i.e. drop down
            }
...
//        add(sh);
        ...
        getContentPane().setBackground(new Color(255,242,179));
    ...
    public void actionPerformed(ActionEvent ae){
        ...
        String f = fh.getSelectedItem(); //for combo box
 //       String s = sh.getSelectedItem(); //for combo box
