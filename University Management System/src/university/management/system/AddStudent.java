...
class AddStudent implements ActionListener{
    ...
    JButton b,b1,b2,b3;
    JComboBox c1,c2; // pull-down box
    
      
    Random ran = new Random();
    long first4 = (ran.nextLong() % 9000L) + 1000L;
    long first = Math.abs(first4); //roll_no.
    ...
    public AddStudent(){
        ...
        f.setLayout(null); //for custom layout
        ...
        id8.setBounds(320,30,500,50);      //where it begins and where it ends   left top right bottom
	...
	lab.setFont(new Font("serif",Font.BOLD,20));
        ...
        String course[] = {"B.E","Law","Architecture","Bsc","Msc","MBA"};
	...
	lab2.setFont(new Font("serif",Font.BOLD,20));
        ...
        String branch[] = {"Computer Science","Information Science","Electrical","Mechanical","Civil"};
        ...
        b.addActionListener(this); //for an event to occur i.e. for next step to occur(value gets stored)
    ...
    public void actionPerformed(ActionEvent ae){
        ...
        String a = t1.getText(); //takes the value
        ...
        String l = (String)c1.getSelectedItem(); //for combo box
        String m = (String)c2.getSelectedItem();//object is returned in combo box so it is typecasted to string
        
        if(ae.getSource() == b){
            try{
                conn cc = new conn();
                String q = "insert into student values('"+a+"','"+bb+"','"+c+"','"+d+"','"+e+"','"+ff+"','"+g+"','"+h+"','"+i+"','"+j+"','"+k+"','"+l+"','"+m+"')";
                cc.s.executeUpdate(q); //insert query
                JOptionPane.showMessageDialog(null,"Student Details Inserted Successfully"); //pop up
                f.setVisible(false); //current frame is closed
            }catch(Exception ee){
